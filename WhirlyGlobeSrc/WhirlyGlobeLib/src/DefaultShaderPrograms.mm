...
static const char *vertexShaderLineNoBackCullingColored =
"uniform mat4  u_mvpMatrix;"
"uniform mat4  u_mvMatrix;"
"uniform mat4  u_mvNormalMatrix;"
"uniform float u_fade;"
...
"   v_dot = dot(-pt.xyz,testNorm.xyz);"
"   v_color = a_color * u_fade;"
"   v_back_color = a_back_color;"
"   gl_Position = u_mvpMatrix * vec4(a_position,1.0);"
"}"
;

static const char *fragmentShaderLineNoBackCullingColored =
"precision mediump float;"
""
"varying vec4       v_color;"
"varying vec4       v_back_color;"
...
"void main()"
"{"
"  gl_FragColor = (v_dot > 0.0 ? v_color : v_back_color);" //vec4(0.5,0.3,0.3,0.1));
"}"
;
...
void SetupDefaultShaders(Scene *scene)
{
    ...
    // Line shader that doesn't do backface culling, but colorize it
    OpenGLES2Program *lineNoBackShaderWithColor = new OpenGLES2Program("Default line shader without backface culling but with backface color",vertexShaderLineNoBackCullingColored,fragmentShaderLineNoBackCullingColored);
    if (!lineNoBackShaderWithColor->isValid())
    {
        NSLog(@"SetupDefaultShaders: Line shader without backface checking (backface colored) didn't compile.");
        delete lineNoBackShaderWithColor;
    }
    else
    {
        scene->addProgram(kToolkitDefaultLineNoBackfaceWithColorProgram, lineNoBackShaderWithColor);
    }
